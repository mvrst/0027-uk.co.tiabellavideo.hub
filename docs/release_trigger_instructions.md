...
- **QA APK:** Download **`app-release.apk`** from  
  ...
  Install on a device via `adb install` or sideload. Same signing as release builds (`0027_upload`). Does **not** replace AAB uploads to Play.
...
4. Fail the build if **no** `*.apk` exists under **`app/build/app/outputs/flutter-apk/`**
5. Artifact glob: **`app/build/app/outputs/flutter-apk/*.apk`**
